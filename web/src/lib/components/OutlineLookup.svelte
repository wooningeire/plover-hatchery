<script lang="ts">
    import {
        ...
        type LookupBreakdownStep,
        type LookupBreakdownTransition,
    } from "$lib/ploverApi";
    ...
    const formatResultTitle = (result: LookupBreakdown) => (
        result.translation ?? "Unknown translation"
    );

    const formatEntryId = (result: LookupBreakdown) => {
    ...
    const stepTransitions = (step: LookupBreakdownStep) => (
        step.transitions ?? []
    );

    const transitionLabel = (transition: LookupBreakdownTransition) => (
    ...
    <div class="lookup-shell">
        ...
        {:else if lookupBreakdownData.length > 0}
            <div class="result-list" aria-live="polite">
                {#each lookupBreakdownData as result}
