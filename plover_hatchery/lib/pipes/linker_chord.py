    ...
    def plugin(get_plugin_api: GetPluginApi, **_):
        ...
        def _(banks_state: BanksState, left_node: "int | None", right_node: "int | None", **_):
            ...
            if sounds.is_vowel(*next_index):
                new_stroke_node = banks_state.trie.get_first_dst_node_else_create(right_node, TRIE_STROKE_BOUNDARY_KEY, TransitionCostInfo(0, banks_state.entry_id))
                banks_state.trie.link_chain(new_stroke_node, left_node, stroke.keys(), TransitionCostInfo(0, banks_state.entry_id))
            else:
                banks_state.trie.link(right_node, left_node, TRIE_STROKE_BOUNDARY_KEY, TransitionCostInfo(0, banks_state.entry_id))
